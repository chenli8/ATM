...
var checkboxTree = Vue.extend({
  ...
  template:[
       ...
       '<Checkbox :value="data.name" style="display:inline-block; margin:3px 3px 3px 0;" :checked.sync="data.checked" v-bind:style="{marginLeft:checkboxMargin(data)}" @change="upadateSelected(data,$event)" :disabled="disabled">{{data.name}}</Checkbox>',
       '<template v-for="obj in data.children">',
       '<pcheck-tree :data="obj" @change="onChange" ></pcheck-tree>',
       '</template>',
       '</div>',
       ].join(''),
  props:{
